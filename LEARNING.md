...
I didn't think I would run into this issue but my backend dependencies were tricky to install and giving me grief. Sometimes it would only work on windows and not mac or vice versa. I ended up getting it working on windows, mac and WSL but the backend dependencies still won't install properly on netlify or github actions. I only need netlify to deploy the frontend, but the backend needs to install and put something into the frontend first (my autogenerated types folder). So this meant that netlify's automated builds would have to install all the dependencies including the backend ones. I ended up switching to manual builds instead, which involves doing everything locally and then deploying a build folder. Lot of painful lessons here.
